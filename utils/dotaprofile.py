...
class DotaProfile:
    def __init__(self, userid):
        self.userid = userid
        self.username = Parse(userid).get_username()
        self.avatar_url = Parse(userid).get_avatar_url()
        self.win_count = Parse(userid).get_won()
        self.lose_count = Parse(userid).get_losses()
        self.rank_tier = Parse(userid).get_rank_tier()
        self.heroes = Parse(userid).get_five_player_heroes_names()
        self.last_match = Parse(userid).get_last_match_time()
        self.is_plus = Parse(userid).is_dota_plus()
        ...
        self.get_leaderboard_rank = str(Parse(userid).get_leaderboard_rank())
    ...
    def __setup_font(self, name, size: int):
        return ImageFont.truetype(f"fonts/{name}.ttf", size, encoding="unic")
    ...
    def __output_images_of_five_favorite_characters(self):
        ...
        for hero in self.heroes:
            if hero.count(" ") != 0:
                hero = hero.replace(' ', '_')
            image_png = Image.open(f'img/heroes/{hero}.png').convert('RGBA')
    ...
    def __rank_images(self):
        ...
        if rank[0] == 'Immortal':
            if int(rank[1]) > 100:
                rank_img = Image.open(f'img/ranks/Immortal.png').convert('RGBA')
                return [rank_img, int(rank[1])]
            elif 100 >= int(rank[1]) > 10:
                rank_img = Image.open(f'img/ranks/Immortalb.png').convert('RGBA')
                return [rank_img, int(rank[1])]
            elif 10 >= int(rank[1]) >= 1:
                rank_img = Image.open(f'img/ranks/Immortalc.png').convert('RGBA')
                return [rank_img, int(rank[1])]
        else:
            rank_img = Image.open(f'img/ranks/{rank[0]}.png').convert('RGBA')
            rank_stars_img = Image.open(f'img/ranks/stars/{rank[1]}.png').convert('RGBA')
